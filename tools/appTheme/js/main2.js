...
function generatePalette(baseColorHSL, size, step = 10) {
  ...
  for (let i = -size; i < size; i++) {
    ...
    let lightness = baseColorHSL.l - i * step;

    if (lightness > 0 && lightness < 100) {
      console.log(lightness);
      palette.push(`hsl(${baseColorHSL.h}, ${baseColorHSL.s}%, ${baseColorHSL.l - i *step}%)`)
    }
  ...
  //paletteContainer.innerHTML += `<h2 class="mt-5">Cores claras</h2>`
  //displayPalette(lightPalette)
  //paletteContainer.innerHTML += `<h2 class="mt-5">Cores escuras</h2>`
  //displayPalette(darkPalette)
  //paletteContainer.innerHTML += `<hr class="my-5">`
}
...
function displayPalette(palette) {
  ...
  //paletteContainer.innerHTML += "<hr>"
  paletteContainer.innerHTML += `<div class="row p-1 gap-0 rounded-3" id="lista_mono${row_id}"></div>`
